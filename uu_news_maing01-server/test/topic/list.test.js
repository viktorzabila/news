...
describe("Testing the topic/list uuCmd...", () => {
  test("HDS", async () => {
    let session = await TestHelper.login("AwidLicenseOwner", false, false);
    let listOne = await TestHelper.executePostCommand("author/create", {name:"dae2", surname:"dae2"});
    let listTwo = await TestHelper.executePostCommand("author/create", {name:"dada", surname:"dae2"});
    let listThree = await TestHelper.executePostCommand("author/create", {name:"adsd", surname:"dae2"});

    let list = await TestHelper.executeGetCommand("author/list", {});
