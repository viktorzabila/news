...
const logger = LoggerFactory.get("NewsMainAbl");
...
class NewsMainAbl {
  ...
  async init(uri, dtoIn, session) {
    ...
    // HDS 2
    const schemas = ["newsMain", "newspaper", "article", "topic", "author"];
    ...
    let uuNews = null;
    try {
      uuNews = await this.dao.create(uuObject);
    } catch (e) {
