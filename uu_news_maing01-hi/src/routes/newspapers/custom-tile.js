...
export const CustomTile = createVisualComponent({
  ...
  render(props) {
    //@@viewOn:private
    const { handleOpenDetailsModal, data: item, handleReload } = props;
          ...
          <div {...attrs}>
            <UU5.Bricks.Card>
                <UU5.Bricks.Dropdown label="..." size="l" colorSchema="gray">
                  ...
                  <UU5.Bricks.Dropdown.Item label="delete" onClick={() => {
                    return confirm.open({
              ...
              <div onClick={() => UU5.Environment.setRoute({url: {useCase:"newspaper", parameters: {newspaperId: item.data.id}}}) }>
                <UU5.Bricks.Text
                  content={item?.data?.name}
                />
              <UU5.Bricks.Text
                content={item?.data?.newspaperUrl}
              />
              </div>
            </UU5.Bricks.Card>
          </div>
